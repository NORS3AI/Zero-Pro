...
const PATCHES = [
  ...
  },
  {
    ...
    changes: [
      ...
      { type: 'feat', text: 'Tab key inserts spaces in the editor (default 3, configurable in Settings > Editor > Tab Size)' },
      { type: 'feat', text: 'Auto-indent — every new line starts indented to match your tab size setting' },
      { type: 'feat', text: 'Shift+Tab unindents the current line by removing leading spaces' },
      { type: 'feat', text: 'Word-style alignment shortcuts — Ctrl+E (centre), Ctrl+L (left), Ctrl+R (right), Ctrl+J (justify)' },
      { type: 'feat', text: 'Ctrl+Click multi-select in the binder — select multiple documents individually (in addition to Shift+Click)' },
      { type: 'feat', text: 'Merge button — combine selected documents in selection order into one, with others moved to Trash' },
      { type: 'feat', text: 'Font Family dropdown in toolbar — Sitka Text (default), Georgia, Times New Roman, Arial, Calibri, Cambria, Garamond, Palatino, Verdana, Courier New' },
      { type: 'feat', text: 'Font Size dropdown in toolbar — select from 8px to 72px, applied inline to selected text' },
      { type: 'feat', text: 'Font Color picker in toolbar — choose any colour and apply it to selected text' },
      { type: 'feat', text: 'Bullet and Numbered list buttons in toolbar' },
      { type: 'feat', text: 'Line Spacing dropdown in toolbar — 1.0x (default), 1.15x, 1.5x, 2.0x, 2.5x, 3.0x per paragraph' },
      { type: 'feat', text: 'Default editor font changed to Sitka Text (with Georgia fallback)' },
      { type: 'feat', text: 'Sitka Text option added to Settings > Editor > Font Family' },
